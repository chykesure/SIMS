...
// ---------------------------------------------------------------------------
// Carousel Data — uses high-quality web images
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Full-Width Image Carousel — clean crossfade, no sliding
// ---------------------------------------------------------------------------
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto aspect-[16/7] w-full overflow-hidden rounded-2xl shadow-2xl sm:aspect-[16/6] lg:aspect-[2.2/1]">
        ...
        <button
          onClick={prev}
          className="absolute left-4 top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/20 text-white shadow-lg backdrop-blur-sm transition-all hover:bg-white/40 hover:shadow-xl sm:left-6"
        ...
        <button
          onClick={next}
          className="absolute right-4 top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/20 text-white shadow-lg backdrop-blur-sm transition-all hover:bg-white/40 hover:shadow-xl sm:right-6"
        ...
        {CAROUSEL_SLIDES.map((_, i) => (
          <button
            ...
            className={cn(
              ...
              i === current ? "w-8 bg-[#C0522B]" : "w-2 bg-slate-300 hover:bg-slate-400"
            )}
          />
        ))}
...
// ---------------------------------------------------------------------------
// Passcode Dialog — shown BEFORE dev login
// ---------------------------------------------------------------------------
    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the platform access code to continue.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="mt-2 space-y-4">
          ...
          <div className="flex justify-center gap-3 pt-2">
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className={cn(
                  "flex h-12 w-12 items-center justify-center rounded-xl border-2 text-xl font-bold transition-all duration-200",
                  i < passcode.length
                    ? error
                      ? "border-red-300 bg-red-50 text-red-500"
                      : "border-[#C0522B] bg-[#C0522B]/5 text-[#C0522B]"
          ...
          {/* Passcode input for mobile and desktop keyboards */}
...
export default function LoginPage() {
  ...
  // Ctrl+Alt+Q keyboard shortcut — shows passcode first
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        if (data.code && data.code.startsWith("TENANT_")) {
          setStatusError({ code: data.code, message: data.message });
          return;
        }
      ...
      toast.error("Login failed", { description: err instanceof Error ? err.message : "An unexpected error occurred" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDevLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Developer login failed", { description: err instanceof Error ? err.message : "Invalid credentials" });
    } finally {
      setDevLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <motion.nav
        ...
        className="sticky top-0 z-50 border-b border-slate-100 bg-white/80 backdrop-blur-lg"
      >
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              style={{ backgroundColor: "#C0522B" }}
              onClick={() => { setShowLoginModal(true); setStatusError(null); }}
            >
              Sign In
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 py-16 text-center sm:px-6 sm:py-20 lg:py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Badge variant="outline" className="mb-6 gap-2 border-orange-200 bg-orange-50 px-3 py-1 text-xs font-medium text-orange-700">
              <span className="h-1.5 w-1.5 rounded-full bg-orange-500" />
              Trusted by 500+ schools across Nigeria
            </Badge>

            <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl lg:text-6xl" style={{ fontFamily: 'Georgia, serif' }}>
              The Complete{" "}
              <span className="relative">
                <span style={{ color: "#C0522B" }}>School Management</span>
                <svg className="absolute -bottom-1 left-0 w-full" viewBox="0 0 200 8" fill="none">
                  <path d="M2 6c40-4 80-4 120-2s60 2 76-2" stroke="#C0522B" strokeWidth="3" strokeLinecap="round" opacity="0.3" />
                </svg>
              </span>{" "}
              Platform
            </h1>
            ...
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-slate-600">
              Manage students, compute results, generate report cards, track admissions,
              and handle school finances — all in one powerful platform built for Nigerian schools.
            </p>
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
              ...
              <Button size="lg" variant="outline" className="h-12 px-8 text-base font-semibold text-slate-700" onClick={() => { setShowLoginModal(true); setStatusError(null); }}>
            ...
            <div className="mt-12 flex items-center justify-center gap-10 sm:gap-14">
              {STATS.map((s) => (
                <div key={s.label} className="text-center">
                  <p className="text-xl font-bold" style={{ color: "#C0522B" }}>{s.value}</p>
      ...
      {/* CAROUSEL SECTION — Full width, dedicated */}
      <section className="relative bg-slate-50/50 px-4 pb-16 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mx-auto max-w-6xl">
          <ImageCarousel />
        </motion.div>
      </section>
      ...
      <section className="bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-relaxed text-slate-600">From student enrollment to result computation, CHYK covers every aspect of school administration.</p>
          </motion.div>
          <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((feature, i) => {
              const Icon = feature.icon;
              return (
                <motion.div key={feature.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group rounded-2xl border border-slate-100 bg-white p-6 transition-all duration-200 hover:border-slate-200 hover:shadow-lg">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl ring-1 ring-slate-100 transition-colors group-hover:ring-slate-200" style={{ backgroundColor: "rgba(192,82,43,0.06)" }}>
                    <Icon className="h-6 w-6" style={{ color: "#C0522B" }} />
                  </div>
      ...
      <section className="border-t border-slate-100 bg-gradient-to-b from-slate-50/80 to-white py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl" style={{ fontFamily: 'Georgia, serif' }}>Why Schools Choose CHYK</h2>
          ...
          <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: Globe, title: "Cloud-Based", desc: "Access your school data from anywhere, on any device. No installations needed." },
              { icon: Smartphone, title: "Mobile Friendly", desc: "Responsive design works perfectly on phones, tablets, and desktops." },
              { icon: ShieldCheck, title: "Secure & Private", desc: "Each school's data is fully isolated. Enterprise-grade security." },
              { icon: HeadphonesIcon, title: "24/7 Support", desc: "Get help whenever you need it. We're always here for you." },
            ].map((item, i) => {
              const Icon = item.icon;
              return (
                <motion.div key={item.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="text-center">
                ...
                <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className={cn("relative flex flex-col overflow-hidden rounded-2xl border bg-white transition-shadow hover:shadow-xl", plan.popular ? "border-amber-300 ring-2 ring-amber-100" : "border-slate-200")}>
                  {plan.popular && (
                    <div className="absolute right-4 top-4">
                      <Badge className="border-amber-500 bg-amber-500 px-2.5 py-0.5 text-[10px] font-bold text-white shadow-sm">MOST POPULAR</Badge>
                    </div>
                  )}
                  <div className="p-8">
                    <div className={cn("flex h-12 w-12 items-center justify-center rounded-xl", plan.popular ? "bg-amber-50" : "bg-slate-50")}>
                      <Icon className={cn("h-6 w-6", plan.popular ? "text-amber-600" : "text-slate-500")} />
                    </div>
                  ...
                  <div className="flex-1 border-t border-slate-100 px-8 py-6">
                    <ul className="space-y-3">
                      {plan.features.map((f) => (
                        <li key={f} className="flex items-start gap-2.5 text-sm text-slate-600">
                          <Check className={cn("mt-0.5 h-4 w-4 shrink-0", plan.popular ? "text-amber-500" : "text-emerald-500")} />
                          <span>{f}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="border-t border-slate-100 px-8 py-6">
                    <Button className={cn("w-full font-semibold", plan.popular ? "bg-amber-500 text-white hover:bg-amber-600" : plan.id === "free" ? "" : "border-slate-200 text-slate-700 hover:bg-slate-50")} variant={plan.id === "free" ? "default" : "outline"} onClick={() => navigate("register")}>
                      {plan.cta}
                      <ArrowRight className="ml-1.5 h-4 w-4" />
                    </Button>
                  </div>
      ...
      <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-20 sm:py-28">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="mt-4 text-base text-slate-400">Join hundreds of schools already using CHYK to manage everything efficiently.</p>
            <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <Button size="lg" className="h-12 px-8 text-base font-semibold text-white shadow-lg" style={{ backgroundColor: "#C0522B" }} onClick={() => navigate("register")}>
                Create Your School Account
                <ArrowRight className="ml-1.5 h-5 w-5" />
              </Button>
              <Button size="lg" variant="outline" className="h-12 border-slate-600 px-8 text-base text-slate-300 hover:bg-white/5" onClick={() => navigate("admission")}>
                Admission Portal
              </Button>
            </div>
          </motion.div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
            <div className="flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-md text-white" style={{ backgroundColor: "#C0522B" }}>
            ...
            <p className="text-xs text-slate-400">&copy; {new Date().getFullYear()} CHYK. All rights reserved.</p>
          </div>
          {/* Dev access - very subtle */}
          <div className="mt-8 text-center">
            <button onClick={() => setShowPasscode(true)} className="inline-flex items-center gap-1 text-[10px] text-slate-300 transition-colors hover:text-slate-400">
      ...
      <Dialog open={showLoginModal} onOpenChange={(open) => { setShowLoginModal(open); if (!open) setStatusError(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl text-white" style={{ backgroundColor: "#C0522B" }}>
                <GraduationCap className="h-5 w-5" />
              </div>
              <div>
                <DialogTitle className="text-xl">Welcome Back</DialogTitle>
                <DialogDescription>Sign in to your school dashboard</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="mt-2 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="text" placeholder="admin@school.com" value={email} onChange={(e) => { setEmail(e.target.value); setStatusError(null); }} required className="pl-10" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input id="password" type="password" placeholder="Enter your password" value={password} onChange={(e) => { setPassword(e.target.value); setStatusError(null); }} required className="pl-10" />
              </div>
            </div>
            <Button type="submit" className="w-full text-white" style={{ backgroundColor: "#C0522B" }} disabled={isLoading}>
              {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Signing in...</> : "Sign In"}
            </Button>
          </form>
          <div className="mt-4 flex items-center justify-between text-sm">
            <button type="button" onClick={() => { navigate("register"); setShowLoginModal(false); }} className="font-medium transition-colors hover:underline" style={{ color: "#C0522B" }}>
              Don&apos;t have an account?
            </button>
            <button type="button" onClick={() => { navigate("admission"); setShowLoginModal(false); }} className="font-medium text-muted-foreground transition-colors hover:underline">
              Admission Portal
            </button>
          </div>
        </DialogContent>
      ...
      {/* PASSCODE VERIFICATION — Shows BEFORE dev login */}
      {showPasscode && !showDevLogin && (
        <PasscodeDialog
          onSuccess={() => {
      ...
      {/* DEVELOPER LOGIN DIALOG — Only shows after correct passcode */}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleDevLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="devEmail" type="email" placeholder="admin@CHYKhool.ng" value={devEmail} onChange={(e) => setDevEmail(e.target.value)} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="devPassword">Password</Label>
              <Input id="devPassword" type="password" placeholder="Developer password" value={devPassword} onChange={(e) => setDevPassword(e.target.value)} required />
            </div>
            <Button type="submit" className="w-full text-white" style={{ backgroundColor: "#C0522B" }} disabled={devLoading}>
              {devLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Authenticating...</> : "Access Dashboard"}
            </Button>
          </form>
        </DialogContent>
      </Dialog>
