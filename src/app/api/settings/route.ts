...
// POST /api/settings - Create record based on type in body
  ...
  try {
    ...
    switch (type) {
      ...
      case "teacher-remark": {
        ...
        const record = await db.teacherRemark.create({ data: { tenantId, remark: remark.trim() } });
        return NextResponse.json(record, { status: 201 });
      }
      case "principal-remark": {
        ...
        const record = await db.principalRemark.create({ data: { tenantId, remark: remark.trim() } });
        return NextResponse.json(record, { status: 201 });
      }
      case "school-settings": {
