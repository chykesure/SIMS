...
export default function ExamView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Get all remarks for the dropdown (not just matched ones)
        if (trRes.ok) {
          const remarks = await trRes.json();
          const allRemarks = remarks.map((r) => r.remark).filter(Boolean);
          setTeacherRemarkOptions(allRemarks);
          const matched = remarks.find((r) => r.session === selectedSession && r.term === selectedTerm);
          setCommentTeacher(matched?.remark || "");
        }
        if (prRes.ok) {
          const remarks = await prRes.json();
          const allRemarks = remarks.map((r) => r.remark).filter(Boolean);
        ...
        if (rsRes.ok) {
          const resumptions = await rsRes.json();
  ...
  async function saveComments() {
    ...
    try {
      ...
      await Promise.all([
        fetch("/api/settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "teacher-remark", remark: commentTeacher }) }),
        fetch("/api/settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "principal-remark", remark: commentPrincipal }) }),
        fetch("/api/settings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "resumption", session: selectedSession, term: selectedTerm, openTerm: commentResumption, nextTerm, noSchoolOpen: Number(commentDaysOpened) || 0 }) }),
      ]);
