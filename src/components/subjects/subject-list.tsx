...
export default function SubjectListView() {
  ...
  const handleDeleteAllSubjects = async () => {
    ...
    try {
      setDeletingAll(true);
      const res = await fetch("/api/subjects?all=true", {
        method: "DELETE",
      });
      if (!res.ok) {
        const data = await res.json();
      ...
      const data = await res.json();
      toast.success(data.message || `Deleted ${data.deletedCount} subject(s)`);
      setDeleteAllOpen(false);
      setDeleteAllConfirmText("");
      fetchSubjects();
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "Failed to delete all subjects"
      );
    } finally {
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  <b>ALL {subjects.length} subjects</b> for your school. This
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  ...
                  disabled={
                    deleteAllConfirmText !== "DELETE ALL" || deletingAll
                  }
                  className="bg-destructive text-white hover:bg-destructive/90"
                >
                  {deletingAll ? "Deleting..." : "Yes, Delete All Subjects"}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
