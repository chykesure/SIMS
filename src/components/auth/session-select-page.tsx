...
export default function SessionSelectPage() {
  const { tenant, user, navigate, setSelectedSession: storeSelectedSession } = useAppStore();
    ...
    async function fetchSessions() {
      try {
        ...
        if (res.ok) {
          ...
          // Filter out any sessions with empty/invalid IDs (causes SelectItem crash)
          const sessionList = raw.filter((s: SessionItem) => s.id && s.id.trim() !== "");
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Persist the selected session in the store for downstream use
        storeSelectedSession(selectedSession);
      ...
      // Fallback: persist session and navigate based on role
      storeSelectedSession(selectedSession);
      const role = (user?.role || "").toUpperCase();
