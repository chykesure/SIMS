...
// Icon map matching planKey to Lucide icon component
...
// Determine plan from student count using DB plans
function getRecommendedPlanFromDB(count: number, plans: DBPlan[]): RecommendedPlanResult | null {
  if (count <= 0 || plans.length === 0) return null;

  // Sort by maxStudents ascending
  const sorted = [...plans].sort((a, b) => a.maxStudents - b.maxStudents);

  // Find the smallest plan that fits the student count
  const fitting = sorted.filter((p) => count <= p.maxStudents);

  const plan = fitting.length > 0 ? fitting[0] : sorted[sorted.length - 1];
  ...
  return {
    ...
    maxUsers: plan.maxUsers >= 999999 ? "Unlimited" : String(plan.maxUsers),
    icon: PLAN_ICON_MAP[plan.id] || Zap,
  };
}
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    fetch("/api/plans")
      ...
      .catch(() => {
        // Silent fail — will show no plan card until plans load
      });
  }, []);

  // ─── Compute recommended plan from DB data ────────────────────────
  const recommendedPlan = studentCount
    ? getRecommendedPlanFromDB(Number(studentCount) || 0, dbPlans)
    : null;

  // ─── Get plan name for Step 3 review ──────────────────────────────
  const getPlanLabel = (count: number): string => {
    if (!dbPlans.length) return "Loading...";
    const plan = getRecommendedPlanFromDB(count, dbPlans);
    return plan ? `${plan.name} Plan` : "Custom Plan";
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {step === 1 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="space-y-5">
                  ...
                  {/* ─── Recommended Plan Card (now uses DB prices) ─── */}
                  {recommendedPlan && (
                    <motion.div
                        ...
                        {studentCount && (
                          <div className="flex items-center gap-4 text-sm text-slate-600">
                            ...
                            <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold" style={{ backgroundColor: "#C0522B15", color: "#C0522B" }}>
                              {getPlanLabel(Number(studentCount))}
                            </span>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-3.5">
                    <div className="flex items-start gap-2.5">
                      ...
                      <p className="text-xs leading-relaxed text-amber-700">
                        <span className="font-semibold">Approval Required:</span> After submission, your Cloud Engineer needs to verify and approve your payment for the Security &amp; Cloud Storage term before your account is activated. This typically takes less than 24 hours.
                      </p>
                    </div>
                  </div>
